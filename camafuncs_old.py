...
def get_length_HLCs(time, High, Low, Close, start_idx, length):
   candle_high = 0
   candle_low = 99999
   candle_close = None
   finished = False
   for idx in range(start_idx, start_idx+length):
      ...
      if Low[idx] < candle_low:
         candle_low = Low[idx]
      if (idx == len(Close)-2) or (idx == start_idx+length-1):
...
def create_cama(time, High, Low, Close, length, initval, func, name):
   ...
   for e in range(length):
      cama.append(Close[0]*(initval+1))
   start_idx = 0
   while True:
      candle_high, candle_low, candle_close, finished = get_length_HLCs(time, High, Low, Close, start_idx, length)
      for idx in range(start_idx, start_idx+length):
