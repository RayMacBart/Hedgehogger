...
def create_cama(time, High, Low, Close, length, initval, func):
   cama = []
   ...
   cama = np.array(cama, dtype='float64')
   return cama

def get_cama_R4(time, High, Low, Close, length):
   def calc_R4(high, low, close):
      return ((high - low) * 1.1 / 2 + close)
   cama_R4 = create_cama(time, High, Low, Close, length, 0.0008, calc_R4)
   cama_R4 = _Array(cama_R4, name='cama_R4')
   return cama_R4

def get_cama_R3(time, High, Low, Close, length):
   def calc_R3(high, low, close):
      return ((high - low) * 1.1 / 4 + close)
   cama_R3 = create_cama(time, High, Low, Close, length, 0.0005, calc_R3)
   cama_R3 = _Array(cama_R3, name='cama_R3')
   return cama_R3

def get_cama_S3(time, High, Low, Close, length):
   def calc_S3(high, low, close):
      return (close - (high - low) * 1.1 / 4)
   cama_S3 = create_cama(time, High, Low, Close, length, -0.0005, calc_S3)
   cama_S3 = _Array(cama_S3, name='cama_S3')
   return cama_S3

def get_cama_S4(time, High, Low, Close, length):
   def calc_S4(high, low, close):
      return (close - (high - low) * 1.1 / 2)
   cama_S4 = create_cama(time, High, Low, Close, length, -0.0008, calc_S4)
   cama_S4 = _Array(cama_S4, name='cama_S4')
   return cama_S4